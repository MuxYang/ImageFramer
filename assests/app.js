...
async function loadBrands() {
  ...
  const fallback = normalizeBrand({
    ...
    folder: "Google",
    fontPrefix: "Google",
    logoCandidates: ["./Google/Logo.svg", "./Google/Google.svg", "./Google.svg"],
...
function normalizeBrand(item) {
  ...
  return {
    ...
    folder,
    fontPrefix,
    logo: String(item.logo || `./${folder}/Logo.svg`).trim(),
...
function collectLogoCandidates(brand) {
  ...
  if (folder) {
    autoPaths.push(`./${folder}/Logo.svg`);
    autoPaths.push(`./${folder}/${folder}.svg`);
  }

  for (const token of tokens) {
    autoPaths.push(`./${token}.svg`);
    autoPaths.push(`./logos/${token}.svg`);
    autoPaths.push(`./${token}/${token}.svg`);
    autoPaths.push(`./${token}/Logo.svg`);
  }
...
function collectFontCandidates(brand, weight) {
  ...
  for (const variant of variantWeights) {
    candidates.push(`./${folder}/${prefix}-${variant}.ttf`);
    candidates.push(`./${folder}/${prefix}-${variant}.otf`);
  }
